...
import { dataAdapter, getUserRole } from '../adapters';
import { type RulerLogEntry } from '../types/RulerTypes';
import { homeContentMap, type UserRole } from '../data/homeContent';
...
function getEffectiveRole(): UserRole {
  ...
  if (stored === 'parent' || stored === 'student' || stored === 'professional' || stored === 'general') {
    return stored;
  }
  return 'general';
}
...
const HomePage: React.FC = () => {
  ...
  const content = homeContentMap[userRole];
  ...
  const handleQuickComplete = async (data: QuickCheckInData) => {
    ...
    try {
      ...
      logger.error('[HomePage] Save failed', { error: String(error) });
    }
  };
      ...
      <div className="parent-home">
        <div className="home-container">
          ...
          <div className="mode-selection">
            ...
            >
              ...
              <div className="mode-content">
                <h2>{content.sos.title}</h2>
                <p>{content.sos.description}</p>
            ...
            >
              <div className="mode-icon" aria-hidden="true">○</div>
              <div className="mode-content">
                <h2>{content.deep.title}</h2>
                <p>{content.deep.description}</p>
          ...
          <div className="home-footer">
            <p>{content.footerTip}</p>
            <p className="footer-sub">{content.footerSub}</p>
          </div>
        </div>
      </div>
    );
