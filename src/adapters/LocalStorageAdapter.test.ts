...
describe('LocalStorageAdapter', () => {
    ...
    describe('認證', () => {
        it('應成功註冊新用戶', async () => {
            const result = await adapter.auth.signUp('test@example.com', 'password123', {
                displayName: '測試用戶',
            });
        ...
        it('應拒絕重複郵箱註冊', async () => {
            await adapter.auth.signUp('test@example.com', 'password123');
        ...
        it('應正常化郵箱大小寫', async () => {
            await adapter.auth.signUp('Test@Example.COM', 'password123');
            const result = await adapter.auth.signIn('test@example.com', 'password123');
        ...
        it('應拒絕不存在的用戶', async () => {
            const result = await adapter.auth.signIn('nobody@example.com', 'password123');
        ...
        it('應正確修改密碼', async () => {
            ...
            const result = await adapter.auth.updatePassword('oldpassword', 'newpassword123');

            expect(result.success).toBe(true);

            // 用新密碼登入應成功
            const login = await adapter.auth.signIn('test@example.com', 'newpassword123');
            expect(login.success).toBe(true);
        });

        it('修改密碼時應拒絕錯誤舊密碼', async () => {
            await adapter.auth.signUp('test@example.com', 'oldpassword');
            const result = await adapter.auth.updatePassword('wrongpassword', 'newpassword123');
        ...
        it('應正確刪除帳號', async () => {
            ...
            const login = await adapter.auth.signIn('test@example.com', 'password123');
    ...
    describe('日誌 CRUD', () => {
        beforeEach(async () => {
            ...
            adapter.clearLogsCache();
            await adapter.initialize();
            await adapter.auth.signUp('loguser@example.com', 'password123');
        });
    ...
    describe('草稿', () => {
        beforeEach(async () => {
            await adapter.auth.signUp('draftuser@example.com', 'password123');
        });
    ...
    describe('緩存', () => {
        it('clearLogsCache 應清除內部緩存', async () => {
            await adapter.auth.signUp('cacheuser@example.com', 'password123');
            await adapter.logs.create({
                emotions: [{ id: 'happy', name: '開心', quadrant: 'yellow', energy: 3, pleasantness: 3 }],
                intensity: 5, bodyScan: null, understanding: null, expressing: null, regulating: null,
                postMood: 'same', timestamp: new Date().toISOString(),
