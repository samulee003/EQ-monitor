...
describe('storage', () => {
    ...
    describe('認證', () => {
        ...
        it('應拒絕不存在的用戶', async () => {
            const result = await auth.signIn('nobody@example.com', 'password123');
        ...
        it('應正確修改密碼', async () => {
            ...
            const result = await auth.updatePassword('oldpassword', 'newpassword123');

            expect(result.success).toBe(true);

            const login = await auth.signIn('test@example.com', 'newpassword123');
            expect(login.success).toBe(true);
        });

        it('修改密碼時應拒絕錯誤舊密碼', async () => {
            await auth.signUp('test@example.com', 'oldpassword');
            const result = await auth.updatePassword('wrongpassword', 'newpassword123');
        ...
        it('應正確刪除帳號', async () => {
            ...
            const login = await auth.signIn('test@example.com', 'password123');
    ...
    describe('日誌 CRUD', () => {
        beforeEach(async () => {
            ...
            clearLogsCache();
            await initialize();
            await auth.signUp('loguser@example.com', 'password123');
        });
    ...
    describe('草稿', () => {
        beforeEach(async () => {
            ...
            clearLogsCache();
            await initialize();
            await auth.signUp('draftuser@example.com', 'password123');
        });
    ...
    describe('緩存', () => {
        it('clearLogsCache 應清除內部緩存', async () => {
            await auth.signUp('cacheuser@example.com', 'password123');
            await logs.create({
                emotions: [{ id: 'happy', name: '開心', quadrant: 'yellow', energy: 3, pleasantness: 3 }],
                intensity: 5, bodyScan: null, understanding: null, expressing: null, regulating: null,
                postMood: 'same', timestamp: new Date().toISOString(),
    ...
    describe('storageService（向後兼容門面）', () => {
        beforeEach(async () => {
            ...
            clearLogsCache();
            await initialize();
            await auth.signUp('svcuser@example.com', 'password123');
        });
