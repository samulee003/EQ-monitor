...
describe('InsForgeAuthService', () => {
  ...
  it('註冊時應呼叫 InsForge Auth 並映射使用者資料', async () => {
    ...
    const result = await insforgeAuthService.signUp(' Test@Example.com ', 'password123', '測試用戶');
  ...
  it('登入已刪除帳號時應立刻登出並拒絕恢復登入狀態', async () => {
    ...
    const result = await insforgeAuthService.signIn('test@example.com', 'password123');
