...
describe('AuthContext', () => {
    ...
    it('應透過 InsForgeAuthService 成功登入', async () => {
        ...
        expect(insforgeAuthService.signIn).toHaveBeenCalledWith('test@example.com', 'password123');
    ...
    it('應透過 InsForgeAuthService 成功註冊並初始化 coach_context', async () => {
        ...
        await waitFor(() => expect(result.current.isLoading).toBe(false));
        await act(async () => {
            const registerResult = await result.current.register('new@example.com', 'password123', '新用戶', true);
            expect(registerResult.success).toBe(true);
        });

        expect(insforgeAuthService.signUp).toHaveBeenCalledWith('new@example.com', 'password123', '新用戶');
