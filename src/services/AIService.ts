...
class AIService {
    ...
    private static MOCK_INSIGHTS: Record<string, AIInsight> = {
        "default": {
            summary: "你可能正在進行一場重要的內在對話。不論此刻感受如何，願意看一眼就已經很不容易。",
            underlyingPatterns: ["情境過載", "尋求認同"],
            suggestedAction: "如果你現在是安全的，可以先把當前任務拆成一個很小的步驟，只做最簡單的一項。",
            empatheticQuote: "「情緒可以是一個訊號，不必急著把它變成答案。」"
        },
        "red": {
            summary: "看起來你可能正經歷很滿、卡住的狀態。身體也許正在提醒你：現在需要先降一點負荷。",
            underlyingPatterns: ["壓力累積", "界限被侵犯", "未滿足的控制感需求"],
            suggestedAction: "如果你現在是安全的，可以先做 5-4-3-2-1 五感接地，或暫時離開現場一分鐘。",
            empatheticQuote: "「在刺激與反應之間，有一個空間；在那個空間裡，我們有選擇權。」— 維克多·弗蘭克",
            colorTheory: "紅色狀態多半代表高能量、不舒服的時刻；先暫停，通常比立刻處理問題更有幫助。"
        },
        "yellow": {
            summary: "你可能正處於比較有活力、較順心的狀態。這股能量可以被好好使用，也值得留意不要一下子用完。",
            underlyingPatterns: ["成就達成", "社交連結", "期待實現"],
            suggestedAction: "可以記下此刻的感受，或把這份能量分一點給今天真正重要的一件小事。",
            empatheticQuote: "「愉快的時刻也可以被溫柔地保存下來。」",
            colorTheory: "黃色狀態代表被點亮、較順心的時刻；適合連結與創造，也適合保留一點餘裕。"
        },
        "blue": {
            summary: "很慢、卡住的感受可能讓你覺得沉重。這不代表你不好，也許只是身體需要被照顧。",
            underlyingPatterns: ["疲憊累積", "失落感", "連結斷裂"],
            suggestedAction: "給自己一個柔軟的擁抱，或進行一件微小但滋養自己的事，如泡杯熱茶。",
            empatheticQuote: "「有些日子你會創造歷史，有些日子你只需要撐過去。」",
            colorTheory: "藍色狀態常提醒人需要休息、陪伴或慢一點；這可以是復原的起點。"
        },
        "green": {
            summary: "你可能正處於比較平靜、能承受的狀態。這份穩定可以被看見，也可以被慢慢保存。",
            underlyingPatterns: ["內在平衡", "感恩連結", "安全感滿足"],
            suggestedAction: "可以用一句話記下：剛剛是什麼讓我比較穩？之後需要時再回來看。",
            empatheticQuote: "「平靜不是沒有波動，而是多了一點可以停靠的地方。」",
            colorTheory: "綠色狀態常與恢復、整合和安全感有關；適合把有用的小做法記下來。"
        }
    };
