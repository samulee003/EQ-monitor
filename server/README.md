...
這是「今心 ImXin」項目的 LINE Bot 後端，基於 RULER 對話式狀態機，為焦慮的父母提供即時情緒覺察陪伴。
...
今心 Bot 是一個基於對話的情緒覺察引導系統。用戶在 LINE 上與 Bot 對話，即可完成完整的 RULER 五步情緒覺察練習：

1. **覺察 (Recognize)** — 掃描身體感受
2. **理解 (Understand)** — 從 100+ 情緒詞彙中精確定位
3. **標記 (Label)** — 為情緒命名並挖掘底層需求
4. **表達 (Express)** — 情緒碎紙機儀式，安全宣洩
5. **調節 (Regulate)** — 呼吸引導 / 5-4-3-2-1 接地 / 正念短引導
