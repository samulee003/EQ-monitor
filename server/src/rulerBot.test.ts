...
describe('rulerBot', () => {
  ...
  describe('完整知心四式流程', () => {
    it('7 步對話：觸發 -> 胸口 -> 焦慮 -> 安全感 -> 碎紙 -> 呼吸 -> 好多了', async () => {
      ...
      expect(r1.text).toContain('第一式：心照');
      ...
      expect(r4.text).toContain('第三式：安神');
    ...
    it('跳過表達步驟', async () => {
      ...
      expect(r5.text).toContain('第四式：動念');
  ...
  describe('會話狀態管理', () => {
    ...
    it('完成後可立即開始新練習', async () => {
      ...
      expect(r.text).toContain('第一式：心照');
    });
  });
