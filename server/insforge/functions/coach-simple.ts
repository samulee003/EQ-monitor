...
function stripCrisisRewardLanguage(content: string): string {
  return content
    .split(/\r?\n/)
    .filter((line) => !/(XP|經驗值|金幣|獎勵|升級|設為今天的小行動|小行動回報)/.test(line))
    .join('\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
}

function stripClinicalCertainty(content: string): string {
  return content
    .replace(/我(?:已經)?診斷(?:出)?你[^。！？\n]*(?:[。！？]|$)/g, '我不能做診斷，但可以陪你整理此刻感受。')
    .replace(/你(?:就是|一定是)[^。！？\n]*(?:憂鬱症|焦慮症|躁鬱症|創傷後壓力)[^。！？\n]*(?:[。！？]|$)/g, '這裡不能判斷診斷；如果這種狀態持續或影響生活，請尋求心理師或醫師協助。')
    .replace(/保證[^。！？\n]*(?:治好|改善|痊癒)[^。！？\n]*(?:[。！？]|$)/g, '我不能承諾治療效果，但可以陪你先做一個可承受的小步驟。')
    .trim();
}

function guardFinalCoachResponse(
  responseText: string,
  context: { crisis: boolean; intent: string }
): string {
  let guarded = stripClinicalCertainty(sanitizeCoachResponse(responseText));

  if (context.crisis || context.intent === 'sos') {
    guarded = stripCrisisRewardLanguage(guarded);
    const crisisSafety =
      '先把安全放在第一位。今心不是緊急救援服務；如果你或身邊的人有立即危險，請先離開危險環境並聯絡 119 或 110。';
    const crisisNextStep =
      '接下來我會為你開啟緊急安定練習：先感覺身體、慢慢呼吸，再選一個不會讓你後悔的小動作。';

    if (!guarded) guarded = crisisNextStep;
    if (!/(119|110|立即危險|緊急救援)/.test(guarded)) {
      guarded = `${crisisSafety}\n\n${guarded}`;
    }
    if (!/緊急安定練習/.test(guarded)) {
      guarded = `${guarded}\n\n${crisisNextStep}`;
    }
  }

  return guarded || '我在。先慢慢吸一口氣，再吐氣。';
}
...
### 第 3 步：看見安全的一步
引導使用者先想「接下來哪一個動作最能保護我，也比較不會讓我後悔？」
...
請只把這些狀態轉成自然教練回覆，不要向使用者列出工具名稱或 trace。`;
    ...
    responseText = guardFinalCoachResponse(responseText, {
      crisis,
      intent: agenticLoop.intent.kind,
    });
