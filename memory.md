...
- Production PWA：`https://today-mood.zeabur.app/?v=nav-original-9a5c5c3#home`
...
- 新增測試鎖住這兩件事，避免之後 merge 又把導覽改回 `安定室 / 紀錄 / 洞察 / 主動教練` 或把右上角改回文字按鈕。
...
- `npm run test:run -- src/components/MainLayout.auth.test.tsx src/pages/CoachPage.test.tsx` → 24 tests passed
- `npx tsc --noEmit` → passed
- `npm run test:run` → 364 tests / 39 files passed
...
- `src/pages/CoachPage.tsx`：Coach 返回按鈕與底部導覽文案。
- `src/pages/CoachPage.test.tsx`：鎖定 Coach 底部導覽原版文案、LINE 綁定碼流程。
...
2. 目前主線 UI 還沒有清楚顯示 LINE 官方帳號名稱、Basic ID 或加好友連結；朋友測試前需要人工告知，或補一個明確 LINE 官方帳號入口。
3. 若要把本分支正式合併回 `main`，先確認 GitHub PR/merge 策略，不要 reset 或覆蓋目前發布修正。
