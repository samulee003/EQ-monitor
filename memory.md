...
- 待合入主線的工作分支：
  - `claude/festive-fermi-fe3154`（領先 main 2 commits，目前 Bot Server production 跑的就是這個分支）
...
**重點：production 兩個服務目前都不對齊 `origin/main`，且彼此走在不同 commit 上**。修改完代碼後一定要記得部署，否則 LINE / PWA 使用者看到的是舊版。
...
**緊急（清掉 production drift）**

1. **Bot**：把 `claude/festive-fermi-fe3154` 合入 `main`（PR 或 fast-forward）。如果不合，下次有人從 main 部署 Bot，會把危機檢測 + production 簽名強制再次抹掉。
2. **PWA**：從 main 重新部署一次，把 `index-CEH332th.js` 換成 main HEAD 的 build。

**產品驗證**

3. 在 LINE 真的測「我撐不下去」確認看到 1925 安心專線、不會走練習流程（剛部署完，可以馬上測）。
4. 做 Agentic Action Loop live API smoke：開始 7 日小陪跑 → 建立小行動 → 回報 partial → 查 `coach_micro_actions`、`coach_gamification_stats`、`coach_agent_traces`。
5. 找 1 位非開發者用手機完整試玩，記錄哪一步不懂、卡住或不安心。
6. LINE 使用者：測 PWA → 加 LINE → 綁定 → LINE 情緒整理 → 回 Coach 問最近記錄；暫不測 LINE 建立小行動。
7. WeChat 使用者：直接開 PWA 網頁，測「網頁記錄 + Coach」，不要要求他先裝 LINE。

**規劃**

8. P1 只根據真回饋補：7 日小陪跑文案、LINE 綁定三步驟圖解、首頁入口分流。
9. P2 再考慮：LINE Bot 小行動、金幣商店、個人排行榜頁、Pro fake-door、WeChat Bot、LINE Push quota 長期監控、正式法律式隱私與免責審稿。
