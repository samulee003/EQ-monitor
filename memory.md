...
- 2026-05-16 23:07 已重新部署 PWA；live bundle `index-DVu8hHQG.js`，正式站手機 smoke 確認強度 8/10 會先顯示「先確認安全」，確認後進入「已記下這一刻」；Coach SOS 可完成「看見安全的一步」並顯示本地收束訊息「安定練習先收在這裡」。
- 2026-05-16 23:07 已重新部署 InsForge `coach` Edge Function；線上 code 回讀確認包含 `guardFinalCoachResponse`、`stripCrisisRewardLanguage`、`crisis_reward_blocked` 與 119 / 110 安全文案；`OPTIONS /coach` 回 204，未授權有效 UUID `POST /coach` 回 401。
