...
**Architecture:** 三層隱私模型中的 Tier 2。用戶完成 RULER 記錄後，客戶端提取元數據（象限標籤、需求標籤、強度）並同步到 InsForge 的 `coach_context` 表；現有 `coach` edge function 升級為在生成回應前先讀取此上下文，使教練對用戶的情緒模式有記憶。Auth Modal 新增隱私同意勾選框，首次登入後觸發一次性歷史數據遷移。
...
- [ ] **Step 3：實作 `src/lib/insforge/coachContext.ts`**
...
- [ ] **Step 2：在 register handleSubmit 內加入驗證**
...
- [ ] **Step 3：把 coachOptIn 傳給 register**
...
- [ ] **Step 4：在表單 JSX 中，confirm password 輸入框之後加入勾選框**
...
- [ ] **Step 5：在同一目錄找到或建立 AuthModal.module.css，加入樣式**
...
- [ ] **Step 1：更新 AuthContextType interface**
...
- [ ] **Step 3：在 register 成功後，初始化 coach_context**
...
- [ ] **Step 3：實作 `src/lib/insforge/localStorageMigration.ts`**
...
- [ ] **Step 5：建立 `src/components/MigrationProgress.tsx`**
...
- [ ] **Step 3：在 context value 中暴露遷移狀態**
...
- [ ] **Step 4：在 App.tsx 渲染 MigrationProgress**
...
## Task 7：完成 RULER 記錄後同步元數據

**Files:**
- Modify: `src/adapters/storage.ts`

- [ ] **Step 1：確認 saveLog 的位置**
...
- [ ] **Step 2：在 storage.ts 頂部加入 import**
...
- [ ] **Step 3：在 saveLog 函數末尾，加入背景元數據同步**
...
- [ ] **Step 4：確認 TypeScript 無錯誤且現有測試仍通過**
...
- [ ] **Step 1：在 coach-simple.ts 頂部加入 coach_context 讀取**
...
- [ ] **Step 2：加入 fetchCoachContext helper**
...
- [ ] **Step 3：在請求處理中讀取 userId 並取得 context**
...
- [ ] **Step 4：將 context 注入 system prompt**
...
- [ ] **Step 1：確認 VITE_INSFORGE_URL 已設定**
...
驗證清單（瀏覽器中）：
- [ ] 開啟 App，點擊登入，看到 Auth Modal 有兩個勾選框
- [ ] 嘗試不勾選第一個框就提交，看到「請先閱讀並同意隱私聲明」錯誤
- [ ] 正常登入，若 localStorage 有舊資料，看到遷移進度畫面
- [ ] 完成一次 RULER 流程，在 InsForge 驗證 coach_context 有更新

- [ ] **Step 5：InsForge 驗證元數據已寫入**
