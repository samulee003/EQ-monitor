...
- 使用者在 Coach 問「我最近怎麼樣」，AI 教練可依近期紀錄整理模式與下一步。
...
| 教練 | `#coach` | 主動 AI 教練、LINE 綁定、SOS、呼吸引導 |
...
Coach 不是普通聊天框，而是主動情緒教練。它應該能讀近期脈絡、看見模式、提出一個可以做的下一步。
...
| Function | 用途 | 狀態 |
|---|---|---|
| `coach` | AI 主動教練、工具調用、緊急安定練習、紀錄保存 | 已部署 / 已驗 |
...
它可以在網頁記錄今天心情，也可以加入 LINE Bot 做一段簡短的知心四式；AI 教練會幫你把最近的情緒線索整理成一個下一步。
