...
- **隱私優先**：原始情緒記錄（日記內容）永遠不離開裝置，AI 教練只讀元數據（象限標籤、需求標籤、強度分數）
...
#### 觸發②：情緒模式偵測（每日排程）
- 條件：`最近 5 筆中 ≥ 3 筆為紅色象限 AND avg_intensity ≥ 7`
...
#### 觸發④：完成記錄後（客戶端即時，永遠免費）
- 觸發：用戶完成 RULER 流程後，客戶端直接呼叫 coach-agent
- 不走排程，不計入每月配額
- 免費 / Pro 用戶均享有
