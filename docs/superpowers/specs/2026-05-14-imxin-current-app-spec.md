...
**版本日期**：2026-05-14  
**對應版本**：`0542992 docs: 記錄 LINE RULER 與排程驗收`  
...
今心是一個以 RULER 情緒覺察框架為核心的開源情緒陪伴工具，讓使用者可以在 LINE 裡完成日常情緒覺察，也可以在 PWA 裡回顧情緒脈絡、查看成長資料，並和主動 AI 教練一起整理下一步。

今心不是心理治療、不是診斷工具，也不是單純聊天機器人。它的定位是「情緒覺察與調節練習工具」：在情緒高漲或混亂時，先幫使用者停一下、把感受說清楚，再選一個比較不傷害自己與他人的下一步。
...
- production LINE RULER 資料流已驗：有效簽名 webhook、完整 RULER、寫入 `agent_ruler_logs`、週報與 Coach 讀到資料
...
- 使用者在 LINE 裡輸入「我今天很焦慮」，Bot 逐步帶他走完 RULER。
- 使用者在 PWA 首頁直接用 Mood Meter 記錄今日心情。
- 使用者打開 Coach 問「我最近怎麼樣」，AI 教練讀取近期紀錄，整理模式與一個下一步。
- 使用者情緒很高時點 SOS，進入 Meta-Moment 暫停、呼吸與安全確認。
...
|---|---|---|---|
| LINE Bot | 日常主要入口 | 留一句話、完成 RULER、綁定帳號 | 對話式引導、Quick Reply、LINE 記錄同步 |
| PWA | 回顧與教練入口 | 記錄、查看歷史、看趨勢、跟 Coach 對話 | Mood Meter、時間軸、成長看板、成就、主動教練 |
...
| View | URL hash | 功能 |
|---|---|---|
| 今日心情 | `#home` | 首頁、Mood Meter、快速記錄、LINE 官方帳號入口、今日教練建議 |
| 記錄回顧 | `#history` | 情緒日誌時間軸、過往紀錄查看 |
| 成長看板 | `#growth` | 情緒趨勢、熱力圖、週報洞察 |
| 成就 | `#achievement` | 成就解鎖與練習里程碑 |
| 教練 | `#coach` | 主動 AI 教練、LINE 綁定、Meta-Moment、呼吸引導 |
...
RULER 是今心的核心方法，完整流程包含：

| 步驟 | 中文定位 | 使用者輸入 | 系統目的 |
|---|---|---|---|
| Recognize | 覺察 | 當下能量與愉悅度、身體線索 | 先知道情緒正在發生 |
| Understand | 理解 | 觸發事件、情境、人事物、需求 | 找到情緒背後的原因與需要 |
| Label | 標記 | 情緒詞與強度 | 用更準確的詞命名感受 |
| Express | 表達 | 一段可不修飾的文字 | 安全釋放，不要求禮貌或邏輯完整 |
| Regulate | 調節 | 呼吸、接地、正念等策略 | 從反應切回可選擇的回應 |
...
- 6 位綁定碼 10 分鐘內有效。
- 綁定後，LINE 完成的 RULER 會寫入可供 Coach 與週報讀取的紀錄表。
...
當使用者輸入高風險或高度失控語句，或主動點 SOS，系統進入 Meta-Moment 流程。
...
- Coach 的工具能力包含查摘要、查趨勢、存 RULER、觸發前端行動。
...
| Function | 用途 | 狀態 |
|---|---|---|
| `coach` | AI 主動教練、工具調用、Meta-Moment、紀錄保存 | 已部署 |
| `weekly-report` | 讀取 RULER 紀錄產生週報 | 已部署 |
...
| `save_ruler_log` | 把明確紀錄請求保存為 RULER 日誌 |
...
| 象限 | 色碼 | 情緒 |
|---|---|---|
| 紅色 | `#C58B8A` | 高能量、低愉悅，例如焦慮、憤怒 |
| 黃色 | `#D5C1A5` | 高能量、高愉悅，例如興奮、期待 |
| 藍色 | `#97A6B4` | 低能量、低愉悅，例如疲憊、沮喪 |
| 綠色 | `#AAB09B` | 低能量、高愉悅，例如平靜、滿足 |
...
- production `/webhook` 使用有效 LINE 簽名可接受完整 RULER 訊息
- `agent_ruler_logs` 寫入 LINE full-flow 資料
- `weekly-report` 可讀取 LINE RULER 資料
- Coach 可讀到最近一筆 LINE RULER 資料
...
- RULER 首頁進入與象限選擇。
- Coach SOS 開啟 Meta-Moment 四步覆蓋層。
- LINE 綁定碼輸入後顯示已綁定。
...
- 找 1 位朋友用手機完整跑：PWA → LINE 加好友 → 輸入綁定 → PWA 貼碼 → LINE RULER → Coach 讀資料。
