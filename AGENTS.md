...
| 服務 | 平台 | 配置檔 |
|---|---|---|
| PWA 前端 | Zeabur（靜態託管） | `zeabur.toml` |
| Bot 後端 | Zeabur / Fly.io / Render / Docker | `server/zeabur.toml`、`server/fly.toml`、`server/render.yaml`、`server/Dockerfile` |
| Edge Functions | InsForge Functions | `server/insforge/functions/` |
...
# 測試
npm run test              # 監視模式
npm run test:run          # 單次執行（265 測試通過）
...
| User API Key (`uak_`) 無法 headless 取得 | 平台限制 | 用 direct SQL + MCP 工具做基礎設施任務 |
