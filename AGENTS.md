...
**PM 判斷**：綠燈給 1-3 位朋友封閉試玩；仍不建議公開宣傳或大量開放。最新可測版本已由 `codex/stitch-ui-polish-20260513` 部署到 production PWA：`https://today-mood.zeabur.app/`。
...
- Playwright E2E 框架已在主線存在，`npm run test:e2e` 目前覆蓋 LINE 綁定、Meta-Moment SOS 與 RULER 關鍵路徑。
- 首頁已回到使用者偏好的原版安靜風格，保留快速記錄與 LINE Bot 入口。
- 頁首右側功能已改為純 SVG 圖示按鈕，避免「勳 / 系 / 訊」這類不明文字。
- LINE Bot 入口已明確顯示正式帳號與加入連結：
  - 顯示名稱：`鋅鋰師拔麻的小小額葉養成手札`
  - Basic ID：`@980pqrhn`
  - 加入連結：`https://line.me/R/ti/p/@980pqrhn`
  - 來源常數：`src/constants/lineBot.ts`
- 從首頁「前往教練綁定」會寫入 `sessionStorage.imxin_focus_line_binding`，進入 Coach 後自動捲到 LINE 綁定卡並聚焦 6 位碼輸入框。
- Coach 頁 LINE 綁定卡已在第一屏顯示「先加入 LINE 官方帳號 → 在 LINE 輸入『綁定』→ 貼上 6 位碼」。

仍需 PM/QA 實測：
- 用真 LINE 帳號加入 `@980pqrhn` 後跑一次完整流程：LINE 輸入「綁定」→ PWA 貼碼 → LINE 完成 RULER → Coach/週報讀到資料。
- LINE 官方帳號顯示名稱目前不是「今心」，朋友可能會困惑；若要正式公開，建議先在 LINE Official Account / Developers 後台改名或補品牌說明。
- 主動推送 / schedule 相關 commit 不屬於今晚 production PWA polish；若要納入公開版本，必須另行驗證與文件同步。
...
# 測試
npm run test              # 監視模式
npm run test:run          # 單次執行（348 測試通過）
npm run test:coverage     # 覆蓋率報告
npm run test:e2e          # Playwright 關鍵路徑（4 測試通過）
...
# 測試
npm run test              # 監視模式
npm run test:run          # 單次執行（117 測試通過）
...
當前狀態：117 測試通過（9 文件），`npm run build` 通過。
...
| PR #20 舊發佈候選資訊 | 已過期 | 目前 production PWA 以 `codex/stitch-ui-polish-20260513` 的部署結果為準，不要只依 PR #20 舊敘述判斷 |
| ESLint warnings | 已知 | `npm run lint` 目前 0 errors / 31 warnings；多為既有 React hooks / non-null assertion 警告 |
| Vite circular chunk warning | 已知 | `npm run build` 可通過；需另排 chunk 策略整理 |
...
| 主動推送（Proactive AI / Cron）| 分支已實作但未納入 PR #20 | 若要發佈需另跑 pg_cron / fallback smoke 與文件同步 |
| E2E 測試框架 | 已建立 | `npm run test:e2e` 已有 4 條 Playwright 關鍵路徑；不能取代真 LINE E2E |
...
npm run test:run         # 前端全量測試（348 測試）
cd server && npm run test:run # 後端全量測試（117 測試）
