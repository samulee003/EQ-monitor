...
- `main` 目前落後 `origin/main` 2 commits，且工作樹尚未整理成 PR；不要在髒工作樹直接 `git pull`。
...
# 測試
npm run test              # 監視模式
npm run test:run          # 單次執行（336 測試通過）
...
| `main` 落後 `origin/main` 2 commits | 未合併 | 目前工作樹 dirty；先整理 diff/PR，再決定 merge/rebase |
| ESLint warnings | 已知 | `npm run lint` 目前 0 errors / 299 warnings |
